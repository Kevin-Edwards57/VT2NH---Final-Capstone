...
//
//  Created by Kevin Edwards on 04/03/2025
//

import SwiftUI
import MapKit

/// Helper model  to represent an event's map location
struct EventLocation: Identifiable {
    let id = UUID()
    let coordinate: CLLocationCoordinate2D
}

/// Bottom sheet view displaying the full detail for a selected event.
/// Includes event name, description, map preview, directions link,
/// "Save Event" to Core Data, and optional external website link.
struct EventDetailSheet: View {
    
    /// Event being displayed
    var event: FBEvent
    
    /// Whether this event has been saved to Core Data
    @State private var isSaved = false
    
    /// Flag to control showing the "Event Saved" alert
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                // Map Preview with Coordinates (if available)
                ...
                // Optional external website link (manual fallback supported)
    ...
    /// Manual fallback for known events that don’t have URLs in Facebook API
    /// - Parameter name: The event name string to match
    /// - Returns: URL string if matched, else nil
